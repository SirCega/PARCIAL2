...
<c:set var="nuevoEstado" value="${param.estadoEvaluador}" />

<c:if test="${not empty idVersion && not empty nuevoEstado}">
    <!-- Actualizar el estado del evaluador en la versión -->
    ...
    <c:if test="${updateEstado > 0}">
        <c:set var="mensaje" value="Estado del evaluador actualizado con éxito." />
        
        <!-- Verificar si hay alguna versión del anteproyecto con estado_evaluador = 'Aprobado' -->
        <sql:query var="checkAprobado" dataSource="${proyectos}">
            SELECT COUNT(*) AS cantidad 
            FROM versionesanteproyecto 
            WHERE estado_evaluador = 'Aprobado' 
        ...
        <c:if test="${checkAprobado.rows[0].cantidad > 0}">
            <!-- Actualizar el campo aprobado_evaluador en anteproyectos a 'Aprobado' -->
            <sql:update var="updateAprobado" dataSource="${proyectos}">
                UPDATE anteproyectos 
                SET aprobado_evaluador = 'Aprobado' 
        ...
        <c:if test="${checkAprobado.rows[0].cantidad == 0}">
            ...
            <sql:update var="updatePendiente" dataSource="${proyectos}">
                UPDATE anteproyectos 
                SET aprobado_evaluador = 'Pendiente' 
                WHERE id = (SELECT id_anteproyecto FROM versionesanteproyecto WHERE id_version = ?);
                <sql:param value="${idVersion}" />
            </sql:update>
        </c:if>
