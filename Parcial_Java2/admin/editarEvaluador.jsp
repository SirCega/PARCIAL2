...
<body>
    ...
    <div class="form-ui">
        <div class="card-container">
            <div class="card">
                <div class="content">
                    <p class="heading">Actualizar Evaluador</p>
                    <div class="container">
                        <c:if test="${param.modifica == null}">
                            <!-- Consulta para obtener los datos del coordinador a editar -->
                            <sql:query var="evaluador" dataSource="${proyectos}">
                                SELECT * FROM usuarios WHERE id_usuario = ? AND id_rol = 4;
                            ...
                            <div class="input-area">
                                <form class="form needs-validation" method="post">
                                    <table class="table">
                                        <c:forEach var="item" items="${evaluador.rows}">
                                            <tr>
                                                <td></td>
                        ...
                        <c:if test="${param.modifica != null}">
                            <sql:update var="result" dataSource="${proyectos}">
                                ...
                                WHERE id_usuario = ${param.id_usuario} AND id_rol = 4;
                            </sql:update>

                            <!-- Verificación del éxito de la actualización -->
                            <c:if test="${result >= 1}">
                                <p class="para">¡Director actualizado con éxito!</p>
                                <a href="verEvaluadores.jsp" class="btn btn-primary">Volver</a>
                            </c:if>
                        </c:if>
                    </div>
                </div>
